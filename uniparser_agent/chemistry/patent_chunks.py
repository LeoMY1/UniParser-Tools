...
def _example_references(text: str) -> list[str]:
    return [
        f"example:{_ascii_digits(match.group(1))}"
...
def build_patent_chunks(units: list[TextUnit]) -> list[PatentChunk]:
    ...
    for unit in units:
        ...
        if unit.type == "table" and (
            _ASSAY_RE.search(text)
            or any(_ASSAY_RE.search(u.text) for u in current.units[-3:])
        ):
            if current.units:
...
def split_oversized_chunk(
    ...
    *,
    ...
    def compact_spectral(unit: TextUnit) -> TextUnit:
        ...
        text = "\n".join(
            part for part in (prefix, f"{match.group(0)} [spectral peaks omitted]") if part
        )
...
def pack_patent_chunks(
    ...
    *,
    ...
    expanded = [
        part
        for chunk in chunks
        for part in split_oversized_chunk(chunk, max_chars=max_chars)
    ]
