...
def _unsupported_measurements(c: LogicalCompound, summary: str) -> list[str]:
    ...
    evidence = f"{c.local_context}\n{activity_tokens}\n{json.dumps(c.activities_json, ensure_ascii=False)}"
    ...
    *,
    ...
    for c in compounds:
        ...
        conflict = bool(hit.get("structure_text_conflict")) or any(
            re.search(r"conflict|冲突|不一致", item, re.IGNORECASE) for item in uncertainties
        )
        ...
        if unsupported:
            ...
            uncertainties.append("Unsupported measurements were removed from the summary: " + ", ".join(unsupported))
