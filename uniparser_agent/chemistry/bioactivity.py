...
def _table_context(chunk_units: list[TextUnit], table_order: int) -> str:
    nearby = [unit.text for unit in chunk_units if unit.type != "table" and abs(unit.order - table_order) <= 5]
    return "\n".join(nearby)[-3_000:]
...
def build_activity_table_inputs(
    ...
    *,
    ...
    chunk_for_unit = {unit.unit_id: chunk for chunk in chunks for unit in chunk.units}
...
def build_bioactivity_prompt(table: ActivityTableInput) -> tuple[str, str]:
    numbered_rows = [{"source_row": index, "text": text} for index, text in zip(table.row_numbers, table.rows)]
    payload = {
        "activity_table": {
...
def attach_bioactivity_records(
    ...
    by_key = {key: compound for compound in compounds for key in {compound.compound_id, compound.label} if key}
