...
_LAYOUT_NOISE_TYPES = frozenset({"pageheader", "pagefooter", "pagebar", "pagenumber", "watermark", "hline"})
...
def _block_text(block: dict[str, Any]) -> str:
    ...
    if isinstance(contents, list) and contents:
        ...
        for content, content_type in zip(contents, types):
            ...
            if normalized_type in {"equation", "equationinline"} and not text.startswith(("$", r"\(", r"\[")):
                text = f"${text}$"
    ...
    for block in blocks or []:
        ...
        if typ == "table":
            ...
            if (
                ...
                for placeholder, content in zip(reversed(placeholders), reversed(contents)):
                    struct = struct.replace(str(placeholder), str(content))
