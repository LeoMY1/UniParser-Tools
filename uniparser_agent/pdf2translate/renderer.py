...
def build_page_rect_map(pdf_path: str | Path) -> dict[int, tuple[float, float]]:
    ...
    try:
        return {i: (float(page.rect.width), float(page.rect.height)) for i, page in enumerate(doc)}
    finally:
        doc.close()
        ...
        if ch == " ":
            ...
            if j < n and not _is_cjk_char(text[j]) and text[j] not in _NO_LINE_START | _NO_LINE_END and text[j] != "\n":
                ...
                while (
                    k < n
                    and text[k] not in " \n"
                    and not _is_cjk_char(text[k])
                    and text[k] not in _NO_LINE_START | _NO_LINE_END
                ):
                    k += 1
