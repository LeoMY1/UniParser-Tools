...
def translate_units(
    ...
    *,
    ...
    batches = [todo[start : start + llm.batch_size] for start in range(0, len(todo), llm.batch_size)]
    ...
    with ThreadPoolExecutor(max_workers=min(llm.max_workers, len(batches))) as pool:
        futures = [pool.submit(_run_batch, idx, batch) for idx, batch in enumerate(batches)]
