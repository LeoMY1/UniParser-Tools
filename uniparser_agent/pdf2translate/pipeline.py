...
def _resolve_output_dir(output_dir: str | Path | None, overwrite: bool) -> Path:
    if output_dir:
        ...
        out = (Path.cwd() / "translate_out").resolve()
    if out.exists():
        if not overwrite:
...
def run_translate_pipeline(
    ...
    *,
    ...
    started = time.time()
    # If pages_tree lives under the output dir, read it before overwrite wipe.
    ...
    if pages_tree_path:
        ...
        parse_result = parse_document(str(src_pdf), output_dir=str(parse_dir), overwrite=True)
