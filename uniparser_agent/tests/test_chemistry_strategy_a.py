...
def test_patent_chunks_keep_examples_claims_assays_separate() -> None:
    ...
    table_parts = [
        chunk for batch in table_batches for chunk in batch if chunk.parent_chunk_id and chunk.units[0].type == "table"
    ]
...
def test_merge_links_prioritizes_fact_and_records_budget_drops() -> None:
    ...
    assert {item["unit_id"] for item in compound.enrich_json["dropped_unit_ids"]} == {"spectra"}
