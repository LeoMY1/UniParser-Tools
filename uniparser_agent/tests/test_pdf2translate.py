...
def test_pages_tree_to_units_supports_v13_contents_nested_items_and_absolute_bbox() -> None:
    data = {
        "pages_tree": [
            [
                {
    ...
    units = pages_tree_to_units(data, page_rect_map={0: (200.0, 400.0)})

    paragraph = next(unit for unit in units if unit.block_type == "paragraph")
    assert paragraph.text == "Energy $E=mc^2$"
    assert paragraph.bbox_norm == {"x1": 0.1, "y1": 0.1, "x2": 0.9, "y2": 0.3}
    assert paragraph.bbox_pdf.as_tuple() == pytest.approx((20.0, 40.0, 180.0, 120.0))
...
def test_majority_vote_terms() -> None:
    entries = majority_vote_terms([("Alpha", "甲"), ("Alpha", "甲"), ("Alpha", "乙"), ("Beta", "丙")])
...
def test_context_fields_in_request_payload() -> None:
    ...
    def fake_chat(*, system_prompt: str, user_content: str) -> str:
        ...
        return json.dumps([{"unit_id": item["unit_id"], "translated_text": f"译{item['unit_id']}"} for item in payload])
...
def test_system_prompt_mentions_glossary() -> None:
    prompt = build_translate_system_prompt(glossary_entries=[GlossaryEntry("LambdaMART", "LambdaMART")])
...
def test_pipeline_uses_unique_output_and_preserves_existing_pages_tree(tmp_path: Path) -> None:
    ...
    assert Path(result["paths"]["output_dir"]) == tmp_path / "out_1"
...
def test_pipeline_requires_pdf_extension(tmp_path: Path) -> None:
    ...
    with pytest.raises(ValueError, match="PDF"):
        run_translate_pipeline(str(bad))
