...
def parse_llm_response(
    ...
    *,
    ...
    for chapter_block in re.findall(r"<chapter>(.*?)</chapter>", response, flags=re.DOTALL):
        ...
        chapter_title = (
            _id_to_text(title_match.group(1).strip(), content_list, image_prefix)
            if title_match
            else ""
        )
        for pair in re.findall(r"<qa_pair>(.*?)</qa_pair>", chapter_block, flags=re.DOTALL):
            ...
            qa_list.append(
                {
                    "question": (
                        _id_to_text(q_match.group(1).strip(), content_list, image_prefix)
                        if q_match
                        else ""
                    ),
                    "answer": a_match.group(1).strip() if a_match else "",
                    "solution": (
                        _id_to_text(s_match.group(1).strip(), content_list, image_prefix)
                        if s_match
                        else ""
                    ),
